...
export default function Actions(props) {
  const { proId, size, count, name, price } = props;
  ...
  const handleDelete = async (event) => {
    await firebase
      ...
      .update({
        cart: cart
          ? FieldValue.arrayRemove({ proId, count, price, size, name })
          : null,
      });
    ...
    <div className="grid px-4 mb-10 rounded-md select-none lg:grid-cols-2 h-88 cards max-h-84 ">
      ...
      <div className="items-center justify-between pt-3 pb-2 text-center md:flex ">
        ...
        <div className="grid items-center gap-5 sm:flex md:justify-center xs:grid-rows-2">
          <button
            disabled={order}
            className={` text-center border-2 text-purple-medium border-gray-medium ${
              order && "opacity-10"
            }`}
            onClick={handleOrder}
          >
          ...
          <button
            className="border-2 text-red-primary border-gray-medium "
